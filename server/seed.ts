...
async function seed() {
  ...
  // One transaction for the whole seed. Without it an interrupted or failing run
  // left a half-populated database -- a municipality and two users but no children
  // -- which then failed on the unique constraint the next time it was run, and
  // looked like a broken application rather than a partial seed.
  await db.transaction(async (tx) => {
    ...
    // Create two daycares
    const [daycare1] = await tx.insert(daycares).values({
    ...
    const [daycare2] = await tx.insert(daycares).values({
    ...
    const [superAdmin] = await tx.insert(users).values({
      name: 'Super Admin',
    ...
    const [admin1] = await tx.insert(users).values({
      name: 'Admin User',
    ...
    const [staff1] = await tx.insert(users).values({
      name: 'Maria Virtanen',
    ...
    const [guardian1] = await tx.insert(users).values({
      name: 'Anna Korhonen',
    ...
    const [guardian2] = await tx.insert(users).values({
      name: 'Mikko Nieminen',
    ...
    const [child1] = await tx.insert(children).values({
    ...
    const [child2] = await tx.insert(children).values({
      name: 'Aatos Nieminen',
    ...
    const [child3] = await tx.insert(children).values({
      name: 'Lilja Mäkinen',
    ...
    // Daycare 1: Trips
    const [trip1] = await tx.insert(trips).values({
    ...
    await tx.insert(tripResponses).values([
      {
        tripId: trip1.id,
    ...
    const [admin2] = await tx.insert(users).values({
      name: 'Tiina Laine',
    ...
    const [staff2] = await tx.insert(users).values({
      name: 'Jukka Virtanen',
    ...
    const [guardian3] = await tx.insert(users).values({
      name: 'Laura Mäkelä',
      email: 'laura@test.fi',
      passwordHash: hashedPassword,
      role: 'guardian',
    ...
    const [child4] = await tx.insert(children).values({
      name: 'Ville Mäkelä',
    ...
    const [child5] = await tx.insert(children).values({
      name: 'Sofia Virtanen',
